function [vec_active_events,dropping_count_su2] = drop_su2(amount_of_su2_to_be_removed,vec_active_events,dropping_count_su2)
...
array_of_su2_to_be_removed = random_array_of_su2_to_be_removed (amount_of_su2_to_be_removed,int_active_secondary2_users);

array_of_positions_to_be_removed = [];
index_of_array_of_positions_to_be_removed = 0;

index_of_array_of_su2_to_be_removed = 1;

    if (total_amount_of_elements > 0)
        for index=1:total_amount_of_elements
                if(amount_of_su2_to_be_removed > 0)
                    if(vec_active_events(index).type == 'y')
                       su2_element = su2_element + 1;
                           if (su2_element == array_of_su2_to_be_removed(index_of_array_of_su2_to_be_removed))
                               index_of_array_of_su2_to_be_removed = index_of_array_of_su2_to_be_removed + 1;
